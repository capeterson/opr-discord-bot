  ...
  async execute(interaction) {
    ...
    if (sub === 'view') {
      ...
      if (!events || events.length === 0) {
        ...
        embed.setFooter({ text: 'Remove entries with /schedule clear <number>' });
      }
    ...
    if (sub === 'clear') {
      ...
      if (!target) {
        return interaction.editReply({ embeds: [buildErrorEmbed(`No scheduled game at position ${number}. Use \`/schedule view\` to see the list.`)] });
      }
